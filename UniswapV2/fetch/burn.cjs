...
// ABI containing only the burn event
const ProjectAbi = [
  {
    anonymous: false,
    inputs: [
...
const csvFilePath = path.join(__dirname, "burn_events.csv");
...
(async () => {
  try {
    ...
    console.log(`Fetching burn events from blocks ${startBlock} to ${endBlock}...`);
    ...
    while (currentBlock < endBlock) {
      ...
      console.log(`Fetching burn events from blocks ${currentBlock} to ${toBlock}...`);

      const filter = pair.filters.Burn();
      const events = await pair.queryFilter(filter, currentBlock, toBlock);

      console.log(`Fetched ${events.length} Burn events from blocks ${currentBlock} to ${toBlock}`);
